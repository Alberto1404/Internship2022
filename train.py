...
def validation(model, val_loader, metric, loss_function, post_trans, args):
	...
	return mean_metric_vals, mean_loss_val


def train(model, train_loader, val_loader, optimizer, metric, loss_function, loss_list_tr, loss_list_ts, metric_list, args):
	dice_val_best = 0.0
	...
	for epoch in epoch_iterator:
		...
		if (epoch  % eval_num == 0):
			dice_val , loss_val= validation(model, val_loader, metric, loss_function, post_trans, args)
...
def main(args):
	...
	plots.save_loss_metric(loss_list_tr, metric_list, loss_list_ts, args)
...
if __name__ == '__main__':
	...
	parser.add_argument('-epochs', required=False, type=int, help='Number of epochs', default=5)
