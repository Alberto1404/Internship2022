...
def compute_metrics_test(metrics_test, metric_multi, metrics_test_p, metric_test_p, metrics_test_h, metric_test_h, cld_metric,cld_metric_p, cld_metric_h, test_labels, prob, args):
	if args.binary:
	...
	if args.binary:
		return [metrics_test], prediction
	else:
		return [metrics_test, metrics_test_p, metrics_test_h], prediction
...
def pipeline_2(model,weights_dir, json_dict, info_dict, test_loader, k, args):
		...
		for test_data in test_loader:
			...
			if args.binary:
				...
				if args.metric == 'softdice':
					...
					"""metric_bin(y_pred=prediction, y=test_labels)
					metric_test = metric_bin.aggregate().item()"""
					dice = compute_metric_binary(torch.stack(prediction).squeeze(), test_labels.squeeze(), 'dice')
					haus = compute_metric_binary(torch.stack(prediction).squeeze(), test_labels.squeeze(), 'haus')
					avg = compute_metric_binary(torch.stack(prediction).squeeze(), test_labels.squeeze(), 'surfdist')
	...
	if args.binary:
		...
		return [dice_portal_mean, dice_portal_std,
				dice_hepatic_mean, dice_hepatic_std,
				dice_mean, dice_std,
				haus_portal_mean, haus_portal_std,
				haus_hepatic_mean, haus_hepatic_std,
